...
@Component({
  ...
  template: ` <div class="">
    <ul class="flex list-none m-0 p-0 gap-3 items-center">
      <li>
        <button
          type="button"
          class="inline-flex w-8 h-8 p-0 items-center justify-center surface-0 dark:surface-800  dark:border-surface-600 rounded"
          (click)="onThemeToggler()"
        >
          <i
            [ngClass]="'pi pi-spin ' + iconClass()"
      ...
      <li class="relative">
        <button
          ...
          class="inline-flex w-8 h-8 p-0 items-center justify-center surface-0 dark:surface-800  dark:border-surface-600 rounded"
        >
          <i class="pi  pi-palette dark:text-white" style="background-color: var(--primary-color); padding: 0.5rem; border-radius: 50%;"></i>
        </button>
        <div
